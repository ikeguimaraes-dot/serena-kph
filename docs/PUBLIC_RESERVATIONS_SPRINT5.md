...
`db.criar_reserva` delegates to one service shared with the agent and private panel for ordinary slotted bookings. It validates São Paulo local date/time, configured booking window/lead time/same-day policy, active tenant slot and weekday, party limits, closures, special events and capacity.
...
- Public/agent payment or special-event reservations route to the team. The authenticated private endpoint retains its existing manual/event/payment/unslotted INSERT contract via an explicit server-side flag, including recorded payment status/value. It validates tenant/event/slot ownership and configured capacities under the same locks, but does not invent absent slots or initiate charges. Unslotted manual reservations cannot be included in a particular slot capacity without an operator assignment. No new payment or deposit logic is included.
...
- 51 offline Python tests: public HTTP semantics/privacy/errors, auth/tenant contract, menu fallback, handoff regressions and correct-unit links.
...
Follow-up: AGENDA_UNCONFIGURED tool output explicitly forbids promising availability, claiming full/closed or confirming a booking and offers the evidenced unit channel. Private legacy compatibility is tested in the disposable restored database, including event ownership/capacity and unslotted paid entries.
