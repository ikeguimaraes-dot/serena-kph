...
async def update_booking_status(pool, reservation_id, rid, status):
    """Reactivation consumes capacity too; use the exact same slot lock."""
    async with pool.acquire() as c:
        async with c.transaction(isolation='read_committed'):
            ...
            if not original:
                return None
            await c.execute('SELECT pg_advisory_xact_lock(hashtextextended($1,0))', f"booking-slot:{rid}:{original['data']}:{original['turno_id']}")
